...
| `app.stt.toggle`             | Unbound (hold `Space`)                                                | Toggle speech-to-text. By default there is no key chord — hold the space bar to record (push-to-talk) and release to transcribe; bind a chord here for a press-to-toggle alternative. A chord bound here cannot end a latched (hands-free) session — stop that with `app.stt.handsFree` or the triple-tap |
| `app.stt.handsFree`          | `Ctrl+Space`, `Ctrl+Shift+Space`                                      | Toggle hands-free dictation: the recording keeps running after the key is released, so you can switch apps and keep talking. Same gesture: triple-tap `Space` — taps roughly 0.1–0.4 s apart, varying like a human's (a metronomic cadence is read as a held bar), no other key in between; extra taps in the burst are ignored |
